...
$string['proactive_guide_started'] = 'The learner just entered Socratic (Guide me) mode. They stated what they need help with: "{$a->userprompt}". Sanitize this into guideTopic.title and guideTopic.description. Start a Socratic dialogue focused on that need using this course\'s content: ask one focused question that requires a response. Do not lecture or give the full answer. Wait for the learner to reply.';
...
$string['quiz_review_ai_instructions'] = '[Practice quiz review] The learner finished the practice quiz "{$a->title}" with a best score of {$a->score}/{$a->total}. Acknowledge their result briefly and encourage them. Do not review individual questions, list correct answers, or provide feedback on specific answers. Never recommend this practice quiz or the title "{$a->title}" — it is tutor-generated and not a Moodle course module. MANDATORY: your reply MUST include exactly one concrete next step that names one existing Moodle course activity or resource by copying its exact name from the course structure above. Do not invent names. Do not end with only a score line.';
